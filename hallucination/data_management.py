...
class Load_fluorescence_microscopy_dataset:
    ...
    def __init__(self, mode, path_train = None, path_val = None, path_test = None):
        ...
        if self.path is not None:
            self.filenames = glob.glob(join(self.path, '*.tif'))
        else:
            self.filenames = [];
    ...
    def __next__(self):
        ...
        if self.count < self.data_size:
            ...
            self.count += 1
            im = np.array(Image.open(fname)).astype(np.float64)
            im /= (256*256-1)
...
class Load_fastMRI_dataset_complex(Load_fastMRI_dataset):
    
    def __init__(self, mode, path_train = None, path_val = None, path_test = None):
        super().__init__(mode, path_train, path_val, path_test)
